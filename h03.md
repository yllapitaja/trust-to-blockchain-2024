...
* A good "one-way hash function" is **collision-free** => It should be hard to generate two pre-images with the same hash value.
...
Installed the needed tools on MacOS Sonoma 14.7. There is no `hashid` available on Mac, but an alternative [`name-that-hash`](https://formulae.brew.sh/formula/name-that-hash):
```
